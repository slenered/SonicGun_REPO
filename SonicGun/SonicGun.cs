...
public class SonicGun : BaseUnityPlugin {
	...
	internal static class Patch {
		...
		private static bool PatchAudioManagerUpdate(AudioManager  __instance) {
			...
			if (_audio == null) {
				_audio = EarSound.PrepareSound(EarSound.TinnitusSound);
			}
			...
			_audio.volume = ValueStorage.tinnitusVolume*0.5f;

			var musicVol = Mathf.Lerp(-80f, 0f, __instance.VolumeCurve.Evaluate( (float)(DataDirector.instance.SettingValueFetch(DataDirector.Setting.MusicVolume) * 0.01f) * (1.2f - ValueStorage.tinnitusVolume)));
			var sfxVol = Mathf.Lerp(-80f, 0f, __instance.VolumeCurve.Evaluate((float)(DataDirector.instance.SettingValueFetch(DataDirector.Setting.SfxVolume) * 0.01f) * (1.2f - ValueStorage.tinnitusVolume)));
			var voiceVol = Mathf.Lerp(-80f, 0f, __instance.VolumeCurve.Evaluate((float)(DataDirector.instance.SettingValueFetch(DataDirector.Setting.ProximityVoice) * 0.01f) * (1.2f - ValueStorage.tinnitusVolume)));
			var ttsVol = Mathf.Lerp(-80f, 0f, __instance.VolumeCurve.Evaluate((float)(DataDirector.instance.SettingValueFetch(DataDirector.Setting.TextToSpeechVolume) * 0.01f) * (1.2f - ValueStorage.tinnitusVolume)));
      ...
      [HarmonyPrefix]
		private static bool PatchReverbDirectorUpdate(ReverbDirector __instance) {
			...
			if (PlayerController.instance.playerAvatarScript.RoomVolumeCheck.CurrentRooms.Count > 0) {
				var reverbPreset = ScriptableObject.CreateInstance<ReverbPreset>();
				var roomReverbPreset = PlayerController.instance.playerAvatarScript.RoomVolumeCheck.CurrentRooms[0].ReverbPreset;
				// print(roomReverbPreset);
				if (roomReverbPreset) {
			...
			if (__instance.lerpAmount < 1f)
			{
				...
				var t = __instance.reverbCurve.Evaluate(__instance.lerpAmount);
